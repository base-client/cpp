...
class AdminSocketClientJobTest : public ::testing::Test {
protected:
	void SetUp() override {
		extern int optind;
		optind = 1;
		...
		int iArgc = 4;
		char *pcArgv[] = {(char *)"./AdminSocketClientJobTest", (char *)"-c", (char *)GstrConfigPath.c_str(), (char *)"-s"};
...
TEST_F(AdminSocketClientJobTest, command_stop)
{
	const int iPid = fork();
	ASSERT_NE(iPid, -1);

	if(iPid == 0) {
		static AdminSocketClientJob adminSocketServerJob;
...
TEST_F(AdminSocketClientJobTest, Start)
{
	ThreadPool threadPool(1);

	AdminSocketClientJob adminSocketServerJob;
...
TEST_F(AdminSocketClientJobTest, Stop)
{
	AdminSocketClientJob adminSocketServerJob;
	EXPECT_TRUE(adminSocketServerJob.Stop());
}
